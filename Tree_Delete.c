...
struct BSTnode* FindMin(struct BSTnode* root)
{
 ...
 //struct BSTnode* Delete(struct BSTnode *root, char data[10]) {

struct BSTnode* Delete(struct BSTnode *root)  {
	if(root == NULL) return root;
	else if(strcmp(NNN,root->Name) < 0) root->left = Delete(root->left);
	else if (strcmp(NNN,root->Name) >0) root->right = Delete(root->right);
...
In_order(a14);
while(1){
printf("\nProvide the Employee Name whose Record you want to Delete\n" );
//scanf("%s",&NNN);
gets(NNN);
Delete(a14);
