...
void In_order(struct BSTnode *root)
{
  ...
  In_order(root->left);
 if (strcmp(root->ssn,SSN)==0) {
    printf("Employee Record of %s:- Name: %s\tSEX: %s\tSALARY: %d\t\n",root->ssn,root->Name,root->sex,root->salary );
  }
  In_order(root->right);
}
...
printf("Provide the Employee SSN whose Record you need\n" );
scanf("%s",&SSN);
